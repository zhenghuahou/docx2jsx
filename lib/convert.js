...
function convertCore(arr = [], cfg = {}) {
  const { matchHdReg = defHdReg, HdTag: _HdTag, BdTag: _BdTag, blackHdReg: _blackHdReg, HdClassName = '',attr = {}} = cfg;
  const rstList = [];
  HdTag = _HdTag || 'h3';
  BdTag = _BdTag || 'p';
  const {title:titleAttr={}} = attr;
  blackHdReg = _blackHdReg || defBlackHdReg;
  const titleAttrStr = Object.keys(titleAttr).reduce((acc,cur)=>{
    const val = titleAttr[cur];
  ...
  arr.forEach((rawStr, index) => {
    ...
    console.info('BdTag:',BdTag,' matchHdReg:',matchHdReg,' r:',r);
    ...
    if (matchStr) {
      ...
      if(HdClassName){
        rstList.push(`<${HdTag} ${titleAttrStr} className="${HdClassName}" title="${title}" >`);
      }else{
        rstList.push(`<${HdTag} ${titleAttrStr} title="${title}">`);
      }
...
function convertToJSX4Inform(rawText = '', cfg = {}) {
  ...
  const list1 = convertCore(arr1, cfg);
  const list2 = convertCore(arr2, {...cfg,BdTag:'Dot'});
